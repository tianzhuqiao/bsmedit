...
{   
	...
	typedef  void(*xsc_callback)(sc_module* pThis, int idx, T value);
public:
    /** Constructor
    ...
    xsc_array():
      ...
      \param name_ module name
        ...
        the callback function is disabled
      */
	  explicit xsc_array( const char* name_ )
		  : sc_object( name_ )
		  ,m_CallBlock(NULL)
		  ,m_funCallback(NULL)
	  { 
      ...
      */
      ...
      \param i the index of the element to be retrived
...
private:
       void init()
       {
           ...
           for(int i=0;i<N;i++)
           {
               ...
               m_signal[i] = new xsc_property<T>(bufName);               
           }
       }
       void uninit()
       {
