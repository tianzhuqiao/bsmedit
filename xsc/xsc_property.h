...
\b Usage
\code
    class somemodule:public tx_module
    {
    public:
         somemodule(sc_module_name name_):
         tx_module(name_)
...
public:
    ...
    xsc_property():
      ...
      \param name_ module name
        ...
        the callback function is disabled
      */
	  explicit xsc_property( const char* name_ )
		  : sc_signal<T>( name_ )
		  ,m_CallBlock(NULL)
		  ,m_funCallback(NULL)
	  {}
      ...
      \param funCallback the callback function

      */
	  explicit xsc_property( const char* name_ ,sc_module* pCallBlock,xsc_callback funCallback)
		  : sc_signal<T>( name_ )
		  ,m_CallBlock(pCallBlock)
      ...
      */
	  void write( const T& value_ )
	  {
		  ...
		  //call back function
		  if(m_funCallback)
			  m_funCallback(m_CallBlock,value_);
		  //update();
	  }
...
private:
	sc_module*   m_CallBlock;
	xsc_callback m_funCallback;
};
