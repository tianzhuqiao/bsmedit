...
class ProcessCommand(object):
    ...
    def response(self, resp):
        if resp.get('ignorable', False) and self.qResp.qsize() > 20:
            # if the resp is ignorable and the qResp is busy, ignore the resp
            return
        resp.pop('ignorable', None)
        self.qResp.put(resp)
            ...
            while True:
                ...
                if cmd:
                    ...
                    if command == 'load':
                        ...
                        # In running mode, such command is generated by the
                        # server, which may flood the client
                        args['ignorable'] = self.running
                    elif command == 'pause':
                        self.running = False
                        resp['value'] = True
